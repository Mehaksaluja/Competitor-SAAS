...
3. **MongoDB** running locally (e.g. `mongodb://localhost:27017`) or set `MONGODB_URI` in `.env`.
...
### 5. Trigger scan (requires scraper)
First install and setup the scraper (see project root `scraper/`), then:
```bash
curl -X POST http://localhost:8000/scan/<competitor_id>
# -> {"ok":true,"message":"Scan started in background","competitor_id":"..."}
```

### 6. Telegram link (optional)
```bash
...
### 7. Alerts (empty until we have change detection)
```bash
